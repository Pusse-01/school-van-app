...
class _Parents_mapState extends State<Parents_map> {
                  ...
                  ? StreamBuilder<QuerySnapshot>(
                      stream: store
                          ...
                          .where(FieldPath.documentId,
                              whereIn: widget.driverids)
                          .snapshots(),
                      builder: (context, locations) {
                        if (locations.connectionState !=
                          ...
                          locations.data!.docs.forEach((element) {
                            ...
                            if (element.get('speed') != null &&
                                element.get('speed').ceil() != 0 &&
                                current?.longitude != null) {
                              double distance = Geolocator.distanceBetween(
                                  element.get('corrds')['lat'],
                                  element.get('corrds')['long'],
                                  current!.latitude,
                                  current!.longitude);
                              time =
                                  ((distance / element.get('speed').ceil() / 60)
                                          .toInt())
                                      .toString();
  ...
  Future locationservice() async {
    ...
    DocumentSnapshot d1 =
        await store.collection('parent').doc(_auth.currentUser!.uid).get();
    ...
    marks.add(
      Marker(
          ...
          position:
              LatLng(d1.get('location')['lat'], d1.get('location')['lon']),
          infoWindow: InfoWindow(
            title: 'Your Location',
          )),
    );
    if (mounted) {
      control?.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(
          target: LatLng(d1.get('location')['lat'], d1.get('location')['lon']),
          zoom: 16)));
      setState(() {});
    }
    ...
    if (widget.driverids.length != 0) {
      store
          .collection('location')
          .where(FieldPath.documentId, whereIn: widget.driverids)
          .snapshots()
          .listen((event) async {
        event.docs.forEach((element) async {
          String time = '-';
          if (element.get('speed').ceil() != 0 &&
              element.get('speed') != null &&
              current?.longitude != null &&
              !widget.notified) {
            double distance = Geolocator.distanceBetween(
                element.get('corrds')['lat'],
                ...
                current!.latitude,
                current!.longitude);
            time = (((distance / element.get('speed')).ceil() / 60).toInt())
                .toString();

            if (int.parse(time) <= 5 && element.get('status')) {
              NotificationService.shownotification(
                  title: '${element.get('name')}',
                  body: 'I\'m Less than 5 min away',
                  payload: 'pick up  notification');
              await store.collection('location').doc(element.id).update({
                'alert': FieldValue.arrayUnion([_auth.currentUser!.uid])
              });
            }
          }
        });
